...
function renderTodolist() {
	...
	for (var i = 0; i < data.todo.length; i++) {
		var valor = data.todo[i];
		addItemTodo(valor);
	}

	for (var j = 0; j < data.completed.length; j++) {
		var valor = data.completed[i];
		addItemTodo(valor, true);
	}
}
...
function removeItem() {
	...
	var id = parent.id;
	var valor = item.innerText;

	if (id === 'todo') {
		data.todo.splice(data.todo.indexOf(valor), 1);
	} else {
	data.completed.splice(data.completed.indexOf(valor), 1);
	}
...
function addItemTodo(text,completed) {
	...
	todo.insertBefore(item, list.childNodes[0]);
}